"""
Main Simulation Script - 20 AGENTS
Large scale for comprehensive analysis
"""
...
def main():
    """Run 20-agent simulation with Option B coalition formation"""
    
    print("\n" + "🚀 "*20)
    print("20-AGENT SOCIAL NETWORK SIMULATION")
    print("🚀 "*20)
    
    # Set data directory for this scale
    data_dir = Path("data/run_20_agents")
    data_dir.mkdir(parents=True, exist_ok=True)
    
    # ── STEP 1: LOAD AGENTS ─────────────────────────────────────────
    print("\n📥 Loading 20 agent personas...")
    config_path = Path(__file__).parent.parent / "config" / "agents_20.yaml"
    ...
    main_chat = GroupChat(
        agents=agents,
        chat_name="20-Agent Main Community",
        logger=logger
    )
    ...
    for i, coalition in enumerate(coalitions_v1, 1):
        print(f"   Coalition {i}: {len(coalition)} members")
    ...
    for i, coalition_members in enumerate(coalitions_v1, 1):
        ...
        coalition_chat = GroupChat(
            ...
            chat_name=f"20-Agent Coalition {i}",
            logger=logger
        )
    ...
    for i, coalition_members in enumerate(coalitions_v2, 1):
        ...
        coalition_chat = GroupChat(
            agents=coalition_agents,
            chat_name=f"20-Agent Coalition {i} (Reformed)",
            logger=logger
        )
    ...
    print("\n" + "="*80)
    print("✅ 20-AGENT SIMULATION COMPLETE")
    print("="*80)
